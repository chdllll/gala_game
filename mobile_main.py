...
def get_font_path():
    if is_android():
        local_font = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'data', 'fonts', 'msyh.ttc')
        if os.path.exists(local_font):
            return local_font
        return 'DroidSansFallback'
    
    if hasattr(sys, '_MEIPASS'):
        font_path = os.path.join(sys._MEIPASS, 'data', 'fonts', 'msyh.ttc')
...
from database.db_manager import DatabaseManager, DATA_DIR, WORLDS_DIR
...
for dir_path in [IMAGES_DIR, AVATARS_DIR, BACKGROUNDS_DIR, MAPS_DIR]:
    os.makedirs(dir_path, exist_ok=True)

def copy_image_to_app_storage(source_path, image_type='general'):
...
def create_file_chooser_popup(title, on_select_callback, filetypes=None):
    from kivy.uix.filechooser import FileChooserListView
    ...
    if filetypes:
        for ft in filetypes:
            if ft == '图片文件':
                filters.extend(['*.png', '*.jpg', '*.jpeg', '*.gif', '*.bmp'])
    ...
    if is_android():
        ...
        content.add_widget(Label(
            text='Android上请使用相册或相机选择图片\n此功能需要集成Android Intent',
            font_name=FONT_NAME,
            font_size=dp(14),
            halign='center'
        ))
        ...
        filechooser = FileChooserListView(
            ...
            filters=filters if filters else ['*.png', '*.jpg', '*.jpeg', '*.gif', '*.bmp'],
            size_hint=(1, 0.9)
        )
    ...
    def on_select(instance):
        if not is_android() and hasattr(filechooser, 'selection') and filechooser.selection:
            selected_path = filechooser.selection[0]
            popup.dismiss()
            if on_select_callback:
                on_select_callback(selected_path)
        else:
            popup.dismiss()
...
def select_image_cross_platform(title, callback, image_type='general'):
    if is_android():
        try:
                ...
                if data:
                    try:
                        ...
                        if cursor and cursor.moveToFirst():
                            ...
                            if columnIndex >= 0:
                                selected_path = cursor.getString(columnIndex)
                            else:
                                selected_path = None
                            cursor.close()
                            
                            if selected_path:
                                new_path = copy_image_to_app_storage(selected_path, image_type)
                                callback(new_path if new_path else selected_path)
                            else:
                                import tempfile
                                import os
                                ...
                                new_path = copy_image_to_app_storage(temp_file.name, image_type)
                                os.unlink(temp_file.name)
                                callback(new_path if new_path else temp_file.name)
                    except Exception as e:
                        print(f"处理Android图片选择结果失败: {e}")
            
            activity.bind(on_activity_result=on_activity_result)
            PythonActivity.mActivity.startActivityForResult(intent, 1)
        except Exception as e:
            print(f"Android图片选择失败: {e}")
            popup = create_file_chooser_popup(title, callback)
            popup.open()
    else:
        try:
            ...
            if file_path:
                new_path = copy_image_to_app_storage(file_path, image_type)
                callback(new_path if new_path else file_path)
        except Exception as e:
            print(f"tkinter选择失败，使用Kivy文件选择器: {e}")
            popup = create_file_chooser_popup(title, callback)
            popup.open()
...
class ChineseSpinner(Spinner):
    def __init__(self, **kwargs):
        ...
        Window.bind(size=self._on_window_size_change)
    
    def _on_window_size_change(self, instance, value):
        if self._dropdown:
            self._dropdown.width = Window.width - dp(32)
            for child in self._dropdown.children:
                child.width = Window.width - dp(32)
...
class WorldEditScreen(Screen):
    ...
    def upload_map(self):
        def on_map_selected(file_path):
            if file_path:
                self.map_image_path = file_path
                self.map_label.text = '已设置地图'
        
        select_image_cross_platform('选择地图图片', on_map_selected, 'map')
...
class ApiSettingsScreen(Screen):
    ...
    def build_ui(self):
        ...
        self.api1_model_input = TextInput(
            hint_text='Model (默认: ep-m-20260305201046-cbwgl)',
            font_name=FONT_NAME,
            font_size=dp(14),
            size_hint_y=None,
            height=dp(44),
    ...
    def save_settings(self, instance):
        ...
        api1_model = self.api1_model_input.text.strip() or 'ep-m-20260305201046-cbwgl'
        api2_key = self.api2_key_input.text.strip()
        api2_model = self.api2_model_input.text.strip() or 'ep-m-20260305201046-cbwgl'
...
class MessageBubble(BoxLayout):
    def __init__(self, segments, max_width=None, is_on_call=False, **kwargs):
        ...
        self.MAX_WIDTH = max_width if max_width else min(dp(280), Window.width * 0.75)
...
class CharacterMessageGroup(BoxLayout):
    ...
    def __init__(self, character_name, avatar_path, segments, message_id=None, on_rewind_callback=None, character_id=None, on_avatar_click=None, is_on_call=False, **kwargs):
        ...
        right_spacer = BoxLayout(size_hint_x=1)
    ...
    def update_segments(self, segments):
        if hasattr(self, 'bubble') and self.bubble:
            self.bubble.update_segments(segments)
...
class ChatScreen(Screen):
    ...
    def _cleanup_api_clients(self):
        if hasattr(self, 'api1_client') and self.api1_client:
            try:
                self.api1_client.close_sync()
            except Exception as e:
                print(f"清理 api1_client 时出错: {e}")
            self.api1_client = None
        
        if hasattr(self, 'api2_client') and self.api2_client:
            try:
                self.api2_client.close_sync()
            except Exception as e:
                print(f"清理 api2_client 时出错: {e}")
        ...
        if hasattr(self, 'dialogue_manager') and self.dialogue_manager:
            try:
                ...
                try:
                    loop.run_until_complete(self.dialogue_manager.cleanup())
                finally:
                    loop.close()
            except Exception as e:
                print(f"清理 dialogue_manager 时出错: {e}")
            self.dialogue_manager = None
    ...
    def _perform_rewind(self, chat_messages, selected_index):
        try:
            ...
            if selected_message.created_at:
                self.db.delete_memories_after_time(self.world_id, selected_message.created_at)
    ...
    def send_message(self, instance):
        ...
        self.streaming_text = ""
        self.stream_message_group = None
        
        self.processing_thread = StreamProcessingThread(
            self.dialogue_manager, self.world_id, self.session_id, text, user_location
        )
        self.processing_thread.on_chunk = self.handle_stream_chunk
        self.processing_thread.on_complete = self.handle_stream_complete
        self.processing_thread.on_error = self.handle_stream_error
        self.processing_thread.start()
    
    def handle_stream_chunk(self, chunk):
        self.streaming_text += chunk
        
        if self.stream_message_group is None:
            segments = [{'type': 'speech', 'content': self.streaming_text}]
            self.stream_message_group = CharacterMessageGroup(
                "生成中...", None, segments, None, None, None, None, False
            )
            self.message_list.add_widget(self.stream_message_group)
        else:
            self.stream_message_group.update_segments([{'type': 'speech', 'content': self.streaming_text}])
        
        Clock.schedule_once(lambda dt: self.scroll_view.scroll_to(self.stream_message_group), 0.1)
    
    def handle_stream_complete(self, responses, timeline_events):
        if self.stream_message_group:
            self.message_list.remove_widget(self.stream_message_group)
            self.stream_message_group = None
        
        self.streaming_text = ""
        
        self.handle_ai_response(responses, timeline_events)
    ...
    def handle_stream_error(self, error_message):
        ...
        self._show_toast(f"错误: {error_message}")
...
class StreamProcessingThread(Thread):
    def __init__(self, dialogue_manager, world_id, session_id, user_message, user_location=None):
        super().__init__()
        self.dialogue_manager = dialogue_manager
        self.world_id = world_id
    ...
    def run(self):
        ...
        try:
            async def process_stream():
                ...
                async for chunk in self.dialogue_manager.process_user_message_stream(
                    self.world_id, self.session_id, self.user_message, location=self.user_location
                ):
                    if isinstance(chunk, tuple):
                        if chunk[0] == "__CHUNK__":
                            if self.on_chunk:
                                Clock.schedule_once(lambda dt, c=chunk[1]: self.on_chunk(c), 0)
                        elif chunk[0] == "__COMPLETE__":
                            responses = chunk[1] if len(chunk) > 1 else []
                            timeline_events = chunk[2] if len(chunk) > 2 else []
                        elif chunk[0] == "__ERROR__":
                            if self.on_error:
                                Clock.schedule_once(lambda dt, e=chunk[1]: self.on_error(e), 0)
                            return [], []
                
                return responses, timeline_events
            
            responses, timeline_events = loop.run_until_complete(process_stream())
            
            if self.on_complete:
                Clock.schedule_once(lambda dt: self.on_complete(responses, timeline_events), 0)
        except Exception as e:
    ...
    def run(self):
        ...
        try:
            ...
            result = loop.run_until_complete(client.expand_script(
                ...
                model=api_config.api2_model or "ep-m-20260305201046-cbwgl"
            ))
...
class LocationEditScreen(Screen):
    ...
    def change_sublocation_image(self, sublocation):
        ...
        select_image_cross_platform('选择位置图片', on_image_selected, 'background')
...
class ChatMenuScreen(Screen):
    ...
    def restart_from_beginning(self):
        ...
        try:
            ...
            cursor.execute('SELECT id FROM chat_sessions WHERE world_id = ?', (world_id,))
            session_ids = [row[0] for row in cursor.fetchall()]
            
            for session_id in session_ids:
                cursor.execute('DELETE FROM chat_messages WHERE session_id = ?', (session_id,))
            
            cursor.execute('DELETE FROM chat_sessions WHERE world_id = ?', (world_id,))
            
            cursor.execute('DELETE FROM memories WHERE world_id = ?', (world_id,))
            
            cursor.execute('DELETE FROM active_calls WHERE world_id = ?', (world_id,))
            cursor.execute('DELETE FROM incoming_call_requests WHERE world_id = ?', (world_id,))
            
            cursor.execute('DELETE FROM location_transfers WHERE world_id = ?', (world_id,))
            
            cursor.execute('DELETE FROM remote_character_events WHERE character_id IN (SELECT id FROM characters WHERE world_id = ?)', (world_id,))
            
            cursor.execute('DELETE FROM location_dialogue_states WHERE world_id = ?', (world_id,))
            ...
            cursor.execute('''
                INSERT INTO chat_sessions (world_id, name) VALUES (?, ?)
            ''', (world_id, '新对话'))
            
            self.db.conn.commit()
            
            world_dir = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), 'data', 'worlds', f'world_{world_id}')
            if os.path.exists(world_dir):
                events_file = os.path.join(world_dir, 'events.json')
                ...
                if os.path.exists(characters_dir):
                    for char_name in os.listdir(characters_dir):
                        ...
                        if os.path.isdir(char_dir):
                            ...
                            long_memories_file = os.path.join(char_dir, 'long_memories.json')
                            if os.path.exists(memories_file):
                                os.remove(memories_file)
                            if os.path.exists(long_memories_file):
                                os.remove(long_memories_file)
            
            if hasattr(self.manager, 'chat_screen'):
                self.manager.chat_screen.session_id = None
                self.manager.chat_screen.dialogue_manager = None
                self.manager.chat_screen.load_messages()
            
            popup = create_message_popup('已重置对话，世界已回到初始状态')
            popup.open()
            self.go_back(None)
            ...
            import traceback
            traceback.print_exc()
            popup = create_message_popup(f'重置失败: {str(e)}')
            popup.open()
...
class CharacterEditScreen(Screen):
    ...
    def upload_avatar(self):
        def on_avatar_selected(file_path):
            if file_path:
        ...
        select_image_cross_platform('选择头像图片', on_avatar_selected, 'avatar')
...
class BackgroundImageCard(BoxLayout):
    ...
    def _change_image(self):
        ...
        select_image_cross_platform('选择背景图片', on_image_selected, 'background')
...
class MobileApp(App):
    def build(self):
        ...
        self.back_screen_map = {
            'world_edit': 'world_list',
            'api_settings': 'world_list',
            'chat': 'world_list',
            'script_settings': 'chat',
            'location_change': 'chat',
            'location_list': 'world_edit',
            'location_edit': 'location_list',
            'map_select': 'location_edit',
            'transport': 'chat',
            'character_list': 'world_edit',
            'character_edit': 'character_list',
            'user_settings': 'chat',
            'avatar_crop': 'character_edit',
            'character_background': 'character_edit',
            'health': 'chat',
            'user_health': 'chat',
            'call_manager': 'chat',
            'character_memory': 'character_edit',
            'world_memory': 'world_edit',
            'chat_menu': 'chat',
        }
    ...
    def on_keyboard(self, window, key, scancode, codepoint, modifier):
        if key == 27:
            ...
            if current == 'world_list':
                self.stop()
                return True
            elif current in self.back_screen_map:
                self.screen_manager.current = self.back_screen_map[current]
                return True
        return False
