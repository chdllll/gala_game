...
class MemorySystem:
                ...
                if should_close_loop:
                    memories_data = loop.run_until_complete(
                        deepseek_client.extract_memory(
                            ...
                            model=config.deepseek_model or "ep-m-20260305201046-cbwgl"
                        )
                    )
    ...
    async def extract_character_memories_async(
        ...
        try:
            api2_client = DeepSeekClient(config.api2_key, "https://ark.cn-beijing.volces.com/api/v3")
            
            result = await api2_client.extract_character_memories(
                world_context=world.background or "",
                chat_messages=chat_messages,
                character_names=character_names,
                model=config.api2_model or "ep-m-20260305201046-cbwgl"
            )
