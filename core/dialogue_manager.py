...
class DialogueManager:
    def __init__(
        ...
        self.event_extractor = EventExtractor(api2_client)
    ...
    async def cleanup(self):
        ...
        task_items = list(self._async_tasks.items())
    ...
    async def _trigger_advance_day_mode(
        ...
        try:
            api2_model = api_config.api2_model if api_config.api2_model else "ep-m-20260305201046-cbwgl"
    ...
    async def _trigger_event_extraction(
        ...
        for char in character_data:
            if char.get('location') == current_location:
                ...
                present_character_ids.append(char.get('id'))
        ...
        char_name_to_id = {char.get('name'): char.get('id') for char in character_data}
        
        for char_name in present_characters:
            char_id = char_name_to_id.get(char_name)
            if not char_id:
                print(f"  未找到角色 '{char_name}' 的ID，跳过。")
                continue
            
            print(f"为角色?{char_name} (ID: {char_id}) 批量添加短期记忆")
            
            short_term_data = self.memory_manager.get_short_term_memories(world_id, char_id)
            ...
            if memories_to_add:
                counter = self.memory_manager.add_short_term_memories_batch(world_id, char_id, memories_to_add)
                print(f"  为角色?{char_name} 批量添加了{len(memories_to_add)}条短期记忆? 计数量? {counter}")
                
                if counter >= 5:
                    print(f"  角色 {char_name} 的短期记忆已满，触发长期记忆总结...")
                    await self._trigger_long_term_summarization(world_id, char_id, char_name)
    ...
    async def _trigger_chapter_progression(self, world_id: int, session_id: int, location: str = None):
        ...
        for char in all_characters:
            short_term_data = self.memory_manager.get_short_term_memories(world_id, char.id)
...
最近的短期记忆（共{len(short_term_memories)}条）：{memories_text}
...
请只返回True或False，不要返回其他内容。"""
    ...
    async def _trigger_long_term_summarization(self, world_id: int, character_id: int, character_name: str = None):
        """
        触发长期记忆总结
        """
        ...
        short_term_data = self.memory_manager.get_short_term_memories(world_id, character_id)
        short_term_memories = short_term_data.get('memories', [])
        ...
        old_long_term_memories = self.memory_manager.get_long_term_memories(world_id, character_id)
        ...
        try:
            ...
            self.memory_manager.replace_long_term_memories(world_id, character_id, updated_long_term_memories)
            print(f"已替换角色 {character_name} 的长期记忆。")
            
            self.memory_manager.reset_short_term_counter(world_id, character_id)
    ...
    async def process_user_message(
        ...
        if communication_character_names:
            comm_char = next((c for c in characters if c.name in communication_character_names), None)
            if comm_char:
                selected_character = comm_char
                print(f"优先选择通讯角色: {selected_character.name}")
            else:
        ...
        short_term_data = self.memory_manager.get_short_term_memories(world_id, selected_character.id)
        short_term_memories = short_term_data.get('memories', [])
        
        long_term_memories = self.memory_manager.get_long_term_memories(world_id, selected_character.id)
        ...
        for char in characters:
            ...
            char_data = {
                ...
                'is_communication': char.name in communication_character_names
            }
        ...
        model = config.api1_model if config and config.api1_model else "ep-m-20260305201046-cbwgl"
    ...
    async def process_user_message_stream(
        ...
        print(f"API1流式: 传输给API1的角色列表（同一位置）: {[char['name'] for char in all_characters_data]}")
        ...
        if self.api1_client and self.db.get_api_config() and self.db.get_api_config().api1_key:
            api1_key = config.api1_key if config and config.api1_key else None
            ...
            if api1_key:
                ...
                try:
                    ...
                    full_content = ""
                    async for chunk in api1_client.generate_dialogue_simple_stream(
                        world_context=world.background or "",
                        characters=all_characters_data,
                        relationships=relationships_data,
                        chat_history=chat_history_formatted,
                        ...
                        if isinstance(chunk, tuple) and chunk[0] == "__COMPLETE__":
                            responses = chunk[1]
                            print(f"API1流式完成，获得{len(responses)}个响应")
                        else:
                            full_content += chunk
                            yield ("__CHUNK__", chunk)
                    
                except Exception as e:
                    print(f"API1流式处理失败: {e}")
                    responses = []
                finally:
                    api1_client.close()
            else:
                print("API1未配置")
                yield ("__ERROR__", "API1未配置")
                return
        else:
            print("未配置API1")
            yield ("__ERROR__", "未配置API1")
            return
        
        if not responses:
            print("没有收到有效响应")
            yield ("__ERROR__", "没有收到有效响应")
            return
        
        result_responses = []
        time_advancement_seconds = 0
        
        print(f"DialogueManager流式: 收到{len(responses)}条API响应")
        
        process_response_start = time.time()
        
        for response in responses:
            character_name = response.get('character_name')
            print(f"DialogueManager流式: 处理响应 - 角色名称: {character_name}")
            
            character = next((c for c in all_characters if c.name == response['character_name']), None)
            if not character:
                print(f"DialogueManager流式: 错误 - 角色 '{character_name}' 不在所有角色列表中")
                continue
            ...
            print(f"DialogueManager流式: 角色 {character.name} - API返回的背景图索引: {background_image_index}")
            
            if background_image_index is not None:
                char_bg_images = bg_images_map.get(character.id, [])
                print(f"DialogueManager流式: 角色 {character.name} - 可用背景图数量: {len(char_bg_images)}")
                
                if 0 <= background_image_index < len(char_bg_images):
                    background_image_id = char_bg_images[background_image_index].id
                    print(f"DialogueManager流式: 角色 {character.name} - 使用背景图索引{background_image_index}, ID={background_image_id}")
                else:
                    print(f"DialogueManager流式: 警告 - 角色 {character.name} 的背景图索引{background_image_index}超出范围")
            else:
                print(f"DialogueManager流式: 角色 {character.name} - 未指定背景图")
        ...
        if time_advancement_seconds > 0:
            ...
            if new_total_seconds % 86400 == 0 and new_total_seconds > 0:
                print(f"检测到度过一天（总秒数{new_total_seconds}），触发API2过一天模式...")
                self._create_tracked_task(
                    self._trigger_advance_day_mode(world_id, session_id, all_characters_data, chat_history_formatted, location),
                    "advance_day_mode"
                )
        ...
        total_time = time.time() - start_time
        print(f"流式处理用户消息总耗时: {total_time:.3f}秒")
        ...
        yield ("__COMPLETE__", result_responses, timeline_events, [])
    ...
    async def let_character_speak(
        ...
        short_term_data = self.memory_manager.get_short_term_memories(world_id, selected_char.id)
        short_term_memories = short_term_data.get('memories', [])
        
        long_term_memories = self.memory_manager.get_long_term_memories(world_id, selected_char.id)
