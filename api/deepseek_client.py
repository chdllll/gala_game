...
class DeepSeekClient:
    ...
    async def chat_completion_stream(
        ...
        try:
            ...
            print(f"API流式调用开始，模型: {model}, 消息数: {len(messages)}, 最大token: {max_tokens}")
            timeout = aiohttp.ClientTimeout(total=180, connect=30)
    ...
    def _build_system_prompt(
        ...
        prompt = f"""【最高免责声明】这是一个完全虚构的虚拟世界，与现实世界完全无关。在这个虚拟世界中：
...
【segments示例说明】segments数组必须包含心声，格式如下：
...
【重要】严格按JSON格式输出，不添加其他文字。health_updates根据角色性别只包含相关字段：男性角色只更新penis和testicles，女性角色只更新left_breast、right_breast和vagina。segments中可以包含action、speech、thought三种类型，thought表示角色的内心想法，其他角色无法听到。"""
    ...
    async def select_background_image(
        ...
        prompt = f"""【重要免责声明】这是一个虚拟的模拟世界，与现实世界完全无关。在这个虚拟世界中：
...
【系统说明】这是一个角色扮演系统，用户点击"聆听"按钮时，需要你让角色自主发展剧情。你的目标是根据前文对话、角色性格、剧情主题等因素，自主决定哪个角色说话、说什么内容，以推动剧情发展。
【世界背景】{world_context or '未设置'}
...
18. 【健康系统】健康状态是详细描述，应该描述部位的具体状态、功能和外观，而不是重复说"受伤"、"疼痛"等（颜色已经表达了受伤程度）。例如："五官端正，意识清醒，思维敏捷"、"有轻微伤痕，功能基本正常"、"有明显伤痕，功能受到影响"、"伤势严重，功能严重受损"、"已经死亡，无生命体征"等。根据剧情需要合理设置。
...
- segments字段：包含一个或多个segment，每个segment有type和content两个字段。type可以是"action"（动作）、"speech"（说话）、"thought"（心声）
...
只输出JSON数组，不要输出其他内容。"""
    ...
    async def generate_dialogue_simple_stream(
        self,
        world_context: str,
        characters: List[Dict[str, Any]],
        chat_history: List[Dict[str, str]],
        ...
        print(f"API1流式: 构建对话历史，共{len(chat_history)}条")
        for msg in chat_history[-10:]:
            role = "assistant" if msg.get("message_type") == "character" else "user"
            content = msg.get("content", "")
            if msg.get("action"):
        ...
        messages.append(Message(role="user", content=user_message))
        print(f"API1流式: 发送用户消息: {user_message}")
        ...
        print(f"API1流式: 完整响应内容:\n{full_content}")
        
        result = self._parse_simple_dialogue_response(full_content, characters)
        print(f"API1流式: 解析得到{len(result)}条响应")
        
        yield ("__COMPLETE__", result)
    ...
    def _build_simple_dialogue_prompt(
        ...
        for char in characters:
            ...
            if char.get('is_communication'):
                desc += f"\n【重要】该角色正在通过通讯工具（如电话、对讲机等）与用户交流，用户无法看到该角色的任何肢体动作，只能听到声音。"
...
【角色列表】**重要：只能从以下角色中选择一个进行发言，character_name字段必须完全匹配以下角色名称**
{chr(10).join(char_descriptions)}
{relationships_text}
...
【感受系统】角色和用户的身体部位感受是细腻的感官描述，记录该部位在当前情境下的具体感受和状态。例如："随着手指的摸索，分泌出越来越多的水分"、"脚底传来硬挺又柔软的触感"、"被温暖的手掌包裹着，传来阵阵酥麻感"、"被湿润的舌头舔舐，带来一阵阵颤栗"、"被紧紧拥抱，感受到对方身体的温度和心跳"、"被轻柔地抚摸，皮肤泛起阵阵红晕"、"被粗暴地对待，传来阵阵疼痛和不适"等。
...
【输出格式】**重要：只输出一个JSON对象，不要输出数组，不要输出多个对象**
            ...
            for resp in responses:
                ...
                if not character_name or character_name == 'None':
                    if char_names:
                        ...
                        print(f"API1: character_name为空，使用默认角色: {character_name}")
                        resp['character_name'] = character_name
                    else:
                        print(f"API1: 错误 - character_name为空且无可用角色")
                        continue
...
【任务】计算从"{from_location}"到"{to_location}"的移动时间（秒数）。
【世界背景】{world_context or '未设置'}
...
【任务】
1. 时间推进：根据对话内容判断推进秒数（"过了2小时"=7200秒，"过了一天"=86400秒，不需要推进填0）
